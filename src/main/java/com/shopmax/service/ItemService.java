...
public class ItemService {
    ...
    public Long saveItem(ItemFormDto itemFormDto,
        ...
        for (int i = 0; i < itemImgFileList.size(); i++) {
            ...
            itemImg.setItem(item); // ★ itemImg가 item을 참조하므로 insert 하기 전 반드시 item 객체를 넣어준다
    ...
    // 상품 가져오기
    @Transactional(readOnly = true) // 트랜잭션 읽기 전용(변경 감지 수행X) -> 성능 향상
    public ItemFormDto getItemDtl(Long itemId) {
        ...
        for (ItemImg itemImg : itemImgList) {
            ...
            itemImgDtoList.add(itemImgDto); // dto 객체를 리스트에 넣는다.
        }
    ...
    public Long updateItem(ItemFormDto itemFormDto,
        ...
        // 5개의 이미지 파일을 업로드했으므로 아래처럼 for문을 이용해 하나씩 이미지 업데이트를 진행
        for (int i = 0; i < itemImgFileList.size(); i++) {
